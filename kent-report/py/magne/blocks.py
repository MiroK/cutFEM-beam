...
def solve_problem(L, p=1, c1 = 1., c2 = 5.,
    ...
    if return_eigs:
        ...
        # Forms
        p_a0 = Constant(c1)*inner(grad(u), grad(v))*d_plate(0) +\
               Constant(c2)*inner(avg(u.dx(0)), avg(v.dx(0)))*d_beam(1)
        p_a1 = Constant(c2)*inner(avg(u.dx(0)), avg(v.dx(0)))*d_beam(1)
        p_s = Constant(1./c2)*inner(avg(u.dx(0)), avg(v.dx(0)))*d_beam(1)

        # Block for preconditioning A0, A1 and S(the schur complement)
        PA0, _ = assemble_system(p_a0, L0, bcs=[bc0])
        PA1, _ = assemble_system(p_a1, L1, bcs=[bc0, bc1], keep_diagonal=True)
        PS, _ = assemble_system(p_s, L0, bcs=[bc0, bc1], keep_diagonal=True)

        # Put the preconditioner together
        # P is [[PA0, 0, 0], [0, PA1, 0], [0, 0, PS]]
        PA0 = to_sparse(PA0)
        PA1 = to_sparse(PA1)
        PS = to_sparse(PS)
        P = bmat([[PA0, None, None], [None, PA1, None], [None, None, PS]])

        # Solve the generalized eigenvalue problem
        from scipy.linalg import eigvalsh
        print "Computing eigenvalues of A size %d" % A.shape[0]
        eigs = eigvalsh(A.toarray(), P.toarray())
