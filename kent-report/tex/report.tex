...
\[
  ...
  % TODO table with results and comment on them


  % -2d poisson problem - weak, galerkin - space as span{phi_i phi_j} -
  % tensor product strucure gives rise to  AUM + MUA = B but phi_i * phi_j
  % is and eigenfunction of laplacian with with eigenvalues. Therefore A = lmbda
  % and OG gives M = 1. The solution is U_ij and the solution is u. As in 1d the
  % method uses an exact inverse on subspace or something similar. Then we need
  % theoretical error estimates and numerical results.
  % results as table
