...
# Shen basis in this case are legendre polynomials combined in such a way that
# the boundary value and the first derivative are zero at [-1, 1]


def _d(k):
    'Weights in lin. comb of Legendre polynomials'
...
def shen_basis(m):
    'Basis in from Legendre polynomials that have u=du=0 at [-1, 1]'
    return [lambda x, i=i:
...
def shen_basis_symbolic(m, x):
    'Basis in from Legendre polynomials that have u=du=0 at [-1, 1]'
    return [_d(i)*(legendre_sym(i, x) -
                   2*(2*i+5)/(2*i+7)*legendre_sym(i+2, x) +
                   (2*i+3)/(2*i+7)*legendre_sym(i+4, x))
            for i in range(m-3)]


def shen_mass_matrix(n):
    'Mass matrix of the Shen basis.'
    M = np.zeros((n, n))
    for k in range(n):
...
def shen_C_matrix(n):
    'Matrix of first derivatices f(1)*g(1) where f, g are Shen basis functions'
    C = np.zeros((n, n))
    for k in range(n):
...
def shen_stiffness_matrix(n):
    'Matrix of second derivatices f(2)*g(2) where f, g are Shen basis functions'
    return np.eye(n)
...
def solve(params, eigs_only=False, fractions=None):
    ...
    The plate and beam deformation are governed by the biharmonic equation.
    We use boundary conditions u=du=0 on the boundary.
    ...
    # Beam length, the beam is parametrized [0, 1] --> beam
    L = np.hypot(*(Q-P))
    assert L > 0
    center = 0.5
    ...
    #                [0,     A1,   B1], * [u1] = [0],
    #                [-B0.T, B1.T, 0]]    [p]]   [0]]
    # A0 block for plate is Ap x Mp + 2*Cp x Cp + Mp x Ap
    Ap = shen_stiffness_matrix(n_plate)
    Mp = shen_mass_matrix(n_plate)
    Cp = shen_C_matrix(n_plate)

    A0 = np.kron(Ap, Mp)*(L1/L0)*(2/L0)**2
    A0 += 2*np.kron(Cp, Cp)*(2/L0)*(2/L1)
    A0 += np.kron(Mp, Ap)*(L0/L1)*(2/L1)**2
