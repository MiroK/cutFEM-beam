...
from try_biharm_2d import solve_lagrange_2d
import numpy as np
from matplotlib import rc
rc('text', usetex=True)
rc('font', family='serif')
import matplotlib.pyplot as plt
from sympy import symbols, lambdify, pi, sin
...
u, test_spec = (x**2*(x-1)**2*sin(pi*x)*y**2*(y-1)**2*sin(-pi*y),
                'test_2d')
domain = [[0, 1], [0, 1]]
E = 1

# Generate problem from specs
problem = manufacture_biharmonic_2d(u=u, E=E, domain=domain)
u = problem['u']
f = problem['f']
...
for i, N in enumerate(Ns):
    start = time.time()
    U, basis = solve_lagrange_2d(f, E=E, domain=domain, m=N)
...
plt.xlabel(r'$N$')
plt.legend(loc='best')
plt.savefig('%s/lagrange_bih_convergence_%s.pdf' % (result_dir, test_spec))
...
plot_u.save('%s/lagrange_bih_u_%s.pdf' % (result_dir, test_spec))
plot_uh.save('%s/lagrange_bih_u_%s.pdf' % (result_dir, test_spec))
